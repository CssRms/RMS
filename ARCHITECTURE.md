...
| `hasIccOverride` / `iccOverrideAmount` | ICC revised the amount post-approval — **takes priority over Audit's figure when both exist** (mirrored in `getEffectiveReqAmount` logic; this precedence rule is duplicated across the PDF generator, the detail modal, and the list table — see §8) |
...
- **Business display logic is duplicated, not shared.** "What's the effective amount for this requisition" and "where is this requisition right now" are each implemented independently in multiple components (`RequisitionsPage.jsx`, `Dashboard.jsx` in at least two places). When you fix one, **grep for the same pattern elsewhere** — it is very likely copy-pasted, not imported from one place, and the other copies will still have the bug.

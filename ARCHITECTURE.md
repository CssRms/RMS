...
└── rms_frontend/
    ...
    │   │   ├── DocumentStudio.jsx ← Word/Excel/PowerPoint editors — ~1,800 lines, all three editors in one file
...
- **Business display logic was duplicated, not shared — partially fixed.** "What's the effective amount for this requisition" and "where is this requisition right now" used to be implemented independently in `RequisitionsPage.jsx` (list table + detail modal) and `Dashboard.jsx` (two places) — four separate copies of the same two rules, each needing its own bugfix. These are now unified into `rms_frontend/src/lib/requisitionDisplay.js` (`getEffectiveAmount`, `getLiveTrailDepartment`) and all four call sites import from there. **What's still duplicated:** `normalizeReq()` (a per-record field-flattening helper) is defined separately in both `RequisitionsPage.jsx` and `Dashboard.jsx`, and the two copies have already drifted — `RequisitionsPage.jsx`'s version has extra fields (`isFromSubAccount`, `deptHeadName`, `parentDeptName`, `visibleToSubAccounts`) that `Dashboard.jsx`'s lacks. Hasn't caused a reported bug yet, but it's the same risk pattern — worth unifying into the shared lib next. **General rule going forward:** when you fix a display bug in one of these files, grep for the same pattern in the other before assuming you're done.
...
- **The `DocumentStudio` JS bundle is ~3.8MB minified** (Vite has warned about this on every build) — no code-splitting between the three editors yet, so loading Document Studio loads Word+Excel+PowerPoint's dependencies all at once regardless of which tab is used.
